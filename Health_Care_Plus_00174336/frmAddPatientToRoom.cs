...
{
    public partial class frmAddPatientToRoom : Form
    {
        ...
        private void frmAddPatientToRoom_Load(object sender, EventArgs e)
        {
            //patients
            try
            {
        ...
        private void dgvRooms_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                DataGridViewRow Row = this.dgvRooms.Rows[e.RowIndex];

                if (Row.Cells[1].Value.ToString() == "Not Available")
                {
                    MessageBox.Show("The Room already using !");
                }
                else
                {
                    tbxRoomNo.Text = Row.Cells[0].Value.ToString();
                }
            }
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                if (Validate())
                {
                    ...
                    MessageBox.Show("Patient admitted to room successfully");
                    manageRooms.LoadRooms(dgvRooms);
                    ClearAll();
                }
            }
        ...
        private bool Validate()
        {
            if (string.IsNullOrWhiteSpace(tbxPatientNo.Text) || string.IsNullOrWhiteSpace(tbxPatientFirstName.Text) || string.IsNullOrWhiteSpace(tbxPatientLastName.Text) ||
                    string.IsNullOrWhiteSpace(tbxRoomNo.Text) ||string.IsNullOrWhiteSpace(tbxProcedures.Text))
            {
